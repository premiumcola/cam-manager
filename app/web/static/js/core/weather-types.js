...
// `de`           = short label shown on chips, badges, gallery pill row
// `de_full`      = full label, used for aria-label / title where space
//                  is not the constraint. Pill renderer falls back to
//                  `de` when `de_full` is absent.
// `is_timelapse` = true when this event-type produces a timelapse,
//                  false (or absent) when it produces a short raw
//                  clip. The dispatcher in weather_service segregates
//                  these into two pipelines today (raw via
//                  EVENT_LABEL_DE in _lifecycle, timelapse via
//                  _event_tl + _sun_tl); the flag is the canonical UI
//                  marker so future surfaces (e.g. a "TL"-tag on the
//                  pill) read from one place. Sunrise / sunset / the
//                  three long-form weather events
//                  (thunder_rising / front_passing / storm_front) are
//                  timelapse-only.
export const WEATHER_TYPES = {
  thunder:    { de: 'Gewitter',        is_timelapse: false, color: '#7faec9',
  ...
  heavy_rain: { de: 'Starkregen',      is_timelapse: false, color: '#5a8aa8',
  ...
  snow:       { de: 'Schnee',          is_timelapse: false, color: '#a8c0d4',
  ...
  fog:        { de: 'Nebel',           is_timelapse: false, color: '#6d7787',
  ...
  sun_timelapse_rise: { de: 'sunrise', de_full: 'Sonnenaufgang (Timelapse)', is_timelapse: true, color: '#e89540',
  ...
  sun_timelapse_set:  { de: 'sunset', de_full: 'Sonnenuntergang (Timelapse)', is_timelapse: true, color: '#d4823a',
  ...
  // werden können.
  thunder_rising: { de: 'Gewitter zieht auf', is_timelapse: true, color: '#7a8eb5',
  ...
  front_passing:  { de: 'Front zieht durch', is_timelapse: true, color: '#9aa5b3',
  ...
  storm_front:    { de: 'Sturmfront', is_timelapse: true, color: '#b08070',
