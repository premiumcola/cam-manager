...
function closeDiscoveryModal() {
  ...
  if (typeof _credAbort !== 'undefined') {
    for (const ac of _credAbort.values()) {
      try {
        ac.abort();
      } catch {}
    }
    _credAbort.clear();
    _credTimers.forEach(clearTimeout);
    _credTimers.clear();
    _credState.clear();
  }
}
let _discoveryItems = [];
function _hostnameToId(h) {
  return h
    .toLowerCase()
    .replaceAll(/[^a-z0-9-]+/g, '-')
    .replaceAll(/^-+|-+$/g, '')
...
function _renderDiscoveryResults() {
  ...
  const visible = hideConfigured
    ? _discoveryItems.filter((x) => !allConfigured.has(x.ip))
    : _discoveryItems.slice();
  ...
  if (!visible.length) {
    byId('discoveryResults').innerHTML =
      `<div class="item">Keine Kamera-Kandidaten${hideConfigured && alreadyCount ? ' (' + alreadyCount + ' bereits konfiguriert ausgeblendet)' : ''}</div>${reolinkHint}`;
    return;
  }
  const showFewHint = _discoveryItems.length < 2;
  byId('discoveryResults').innerHTML = visible
    .map((x) => {
      const ports = (x.open_ports || []).join(', ') || '—';
      const uid = x.ip.replaceAll('.', '_');
      const already = allConfigured.has(x.ip);
      const vendor =
        x.guess === 'Unbekannte Kamera'
          ? `Unbekannte Kamera (${x.ip})`
          : esc(x.guess || 'Unbekannte Kamera');
      const displayName = x.hostname
        ? esc(x.hostname.charAt(0).toUpperCase() + x.hostname.slice(1))
        : '';
      const defaultPath = _defaultRtspPath(x);
      const pathOptsForCam = RTSP_PATHS.map(
        (p) =>
          `<option value="${esc(p.value)}"${p.value === defaultPath ? ' selected' : ''}>${esc(p.label)}</option>`,
      ).join('');
      const reolinkNote = x.reolink_hints
        ? `<div class="field-help" style="padding:3px 6px;margin-top:2px">Reolink: H.264-Main für RLC-810A / ältere FW · H.265-Main für CX810 / neuere FW · Sub immer H.264</div>`
        : '';
      ...
      ${
        already
          ? ''
          : `<div id="disc_form_wrap_${uid}" data-disc-ip="${esc(x.ip)}">
        ${reolinkNote}
...
function _credPaint(ip) {
  ...
  if (st.state === 'probing') txt = 'Prüfe Zugangsdaten …';
    ...
    txt = 'Konnte nicht eindeutig prüfen — speichern auf eigene Verantwortung';
  ...
  _credSetSaveButton(
    ip,
    st.state === 'probing' ? 'probing' : st.state === 'bad' ? 'disabled' : 'enabled',
  );
}
...
async function _credProbeNow(ip) {
  ...
  if (!pw) {
    ...
    const ac = _credAbort.get(ip);
    if (ac) {
      ac.abort();
      _credAbort.delete(ip);
    }
    _credPaint(ip);
    return;
  }
  // Cancel any in-flight probe for this IP first.
  const prev = _credAbort.get(ip);
  if (prev) {
    try {
      prev.abort();
    } catch {}
  ...
  try {
    ...
    d = { ok: false, reason: 'error', detail: '' };
  }
...
function _logPush(html, opts) {
  const panel = byId('discoveryLogPanel');
  if (!panel) return;
...
function _setProgress(scanned, total, currentIp) {
  const wrap = byId('discoveryProgressWrap');
  if (!wrap) return;
  wrap.style.display = 'block';
  const pct = total ? Math.min(100, Math.round((scanned / total) * 100)) : 0;
  const fill = byId('discoveryProgressFill');
...
byId('discoverBtn')?.addEventListener('click', () => {
  ...
  es.addEventListener('phase', (ev) => {
    ...
    if (d.phase === '1') {
      _logPush(
        `<strong>Phase 1</strong> · Subnetz ${esc(d.subnet)} · ${d.total_hosts} Hosts werden gescannt`,
      );
    } else if (d.phase === '2') {
      _logPush(
        `<strong>Phase 2</strong> · ${d.total_hosts} Kandidat${d.total_hosts === 1 ? '' : 'en'} werden geprüft`,
      );
    }
  });
  ...
  es.addEventListener('candidate', (ev) => {
    ...
    if (idx >= 0) _discoveryItems[idx] = d;
    else _discoveryItems.push(d);
  ...
  es.addEventListener('done', (ev) => {
    ...
    byId('discoveryStatus').innerHTML =
      `Subnetz <strong>${esc(d.subnet || _discoverySubnet)}</strong> · ${total} Hosts · <strong>Gefundene Geräte (${found})</strong>`;
    _logPush(
      `<strong>Fertig</strong> · ${found} Kamera${found === 1 ? '' : 's'} gefunden in ${total} Hosts`,
    );
    _renderDiscoveryResults();
    _closeDiscoveryStream();
  });
...
async function _legacyDiscoverFallback() {
  try {
    ...
    byId('discoveryStatus').innerHTML =
      `Subnetz <strong>${esc(r.subnet)}</strong> · ${total} Hosts · <strong>Gefundene Geräte (${found})</strong>`;
    _renderDiscoveryResults();
  } catch (err) {
...
byId('discoveryModal').onclick = (e) => {
  if (e.target === byId('discoveryModal')) closeDiscoveryModal();
};
...
window.applyDiscoveryRtsp = (ip) => {
  ...
  const rtsp = `rtsp://${user}:${_rtspEnc(pass)}@${ip}:554${path}`;
  const snap = `http://${user}:${_rtspEnc(pass)}@${ip}/cgi-bin/snapshot.cgi`;
  const wizRtsp = byId('wiz_cam_rtsp');
  if (wizRtsp) wizRtsp.value = rtsp;
  const wizSnap = byId('wiz_cam_snapshot');
  if (wizSnap) wizSnap.value = snap;
  closeDiscoveryModal();
};
byId('cameraForm').onsubmit = async (e) => {
  ...
  const payload = {
    ...
    telegram_enabled: f['telegram_enabled']
      ? f['telegram_enabled'].checked
      : (existingCam?.telegram_enabled ?? true),
    mqtt_enabled: f['mqtt_enabled']
      ? f['mqtt_enabled'].checked
      : (existingCam?.mqtt_enabled ?? true),
    whitelist_names: getWhitelistState().filter(Boolean),
    timelapse: existingCam?.timelapse || {
      enabled: false,
      fps: 25,
      period: 'day',
      daily_target_seconds: 60,
      weekly_target_seconds: 180,
      telegram_send: false,
    },
    ...
    schedule: (() => {
      ...
      return {
        enabled: n || r,
        from: useNotify
          ? f['schedule_notify_from']?.value || '21:00'
          : f['schedule_record_from']?.value || '00:00',
        to: useNotify
          ? f['schedule_notify_to']?.value || '06:00'
          : f['schedule_record_to']?.value || '23:59',
    ...
    recording_enabled: f['recording_enabled']
      ? !!f['recording_enabled'].checked
      : existingCam?.recording_enabled !== false,
    ...
    motion_enabled: f['motion_enabled']
      ? f['motion_enabled'].checked
...
export function _bindCamProbeDeviceInfo() {
  const btn = byId('camProbeDeviceInfo');
  ...
  btn.addEventListener('click', async () => {
    ...
    if (cur.manufacturer || cur.model) {
      const msg =
        `Mit erkanntem Wert überschreiben?\n\n` +
        `Aktuell: '${cur.manufacturer || '—'}' / '${cur.model || '—'}'\n` +
        `Neu: '${d.manufacturer}' / '${d.model}'`;
      if (!(await showConfirm(msg))) return;
    }
    if (f['manufacturer']) {
      f['manufacturer'].value = d.manufacturer;
      f['manufacturer'].dispatchEvent(new Event('input', { bubbles: true }));
    }
    if (f['model']) {
      f['model'].value = d.model;
      f['model'].dispatchEvent(new Event('input', { bubbles: true }));
    }
