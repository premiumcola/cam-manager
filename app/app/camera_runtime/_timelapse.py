    ...
    def _finalize_timelapse_window(
        ...
        if n < 2:
            ...
            try:
                ...
                log_tl.debug(
                    "[%s][%s] cleaned sparse dir: %s", self.camera_id, profile_name, frames_dir.name
                )
            except Exception as e:
        ...
        if path:
            ...
            try:
                ...
                log_tl.debug(
                    "[%s][timelapse] sidecar JSON written: %s", self.camera_id, meta_path.name
                )
            except Exception as e:
            ...
            try:
                ...
                thumb_rel = (
                    f"timelapse/{self.camera_id}/{stem}.jpg" if thumb_path.exists() else None
                )
                tl_event = {
                    "event_id": f"tl_{stem}",
                    "camera_id": self.camera_id,
        ...
        while self.running:
            ...
            if frame is not None:
                try:
                    ...
                    log_tl.debug(
                        "[%s] timelapse frame saved: %s (interval=%.2fs, q=%d)",
                        self.camera_id,
                        out.name,
                        interval_s,
                        jpeg_q,
        ...
        while self.running:
            ...
            if profile_name == "custom":
                if window_key is None:
                    ...
                    log_tl.info(
                        "[%s][timelapse] custom window started: %s (period=%ds interval=%.0fs)",
                        self.camera_id,
                        window_key,
                        period_s,
                        interval_s,
                    ...
                    if frame_ts == _last_frame_ts:
                        ...
                        if not ok:
                            ...
                            if (
                                ...
                                out = tl_dir / f"{ts}.jpg"
                                cv2.imwrite(
                                    str(out), frame, [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_q]
                                )
